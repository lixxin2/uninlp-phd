...
import edu.hitsz.nlp.mstjoint.Util;
...
public class MSTReader extends DependencyReader {
    ...
    public DependencyInstanceJoint getNext() throws IOException {
		...
		for(int i = 0; i < forms.length; i++) {
		    ...
		    pos_new[i+1] = pos[i];
		    deprels_new[i+1] = labeled ? deprels[i] : "ROOT";
		    heads_new[i+1] = heads[i];
		}

		DependencyInstanceJoint instance =
		    new DependencyInstanceJoint(forms_new, pos_new, deprels_new, heads_new);
    ...
    public DependencyInstanceJoint getNextBlank() throws IOException {
