...
public class MertCore
{
  ...
  private int numDocuments;
    // number of documents in the dev set，文档的数目
  ...
  private int refsPerSen;
    // number of reference translations per sentence，每个句子对应的ref（正确结果）文件数目
  ...
  private int numParams;
    // number of features for the log-linear model，参数的数目
  ...
  private String[] paramNames;
    // feature names, needed to read/create config file，特征名，子模型名
  ...
  private String decoderCommand; 
    // the command that runs the decoder; read from decoderCommandFileName，解码的命令
  ...
  private int sizeOfNBest;
    // size of N-best list generated by decoder at each iteration，每次迭代解码器产生的N-best个结果
  ...
  private int initsPerIt;
    // number of intermediate initial points per iteration，每次迭代中间初始点的数目

  private int maxMERTIterations, minMERTIterations, prevMERTIterations;
    // max: maximum number of MERT iterations，最大迭代次数
  ...
  private String metricName;
    // name of evaluation metric optimized by MERT，采用的评估标准
  ...
  private String dirPrefix; // where are all these files located?文档位置
  //子模型参数文件，
  ...
  //解码的配置文件（候选文件，候选数目，最优数目， 子模型的初始权重），解析命令文件（选择nbest个文件）
...
//  private int useDisk;
  ...
  /**
   * 初始化，包括
   * <li> 设置文档信息: 统计句子数，每个句子对应的文档数，
   * <li> 读取参数文件params.txt, 读取子模型名，初始化子模型的权重lambda,
   * <li> 读取ref.all参考文件, 及参考项（正确项）
   * <li> 读取SDecoder_cmd命令，
   * <li> 初始化EvaluationMetric（参考项），
   * <li> 初始化IntermediateOptimizer
   * @since Nov 6, 2012
   * @param randsToSkip 随机因子
   */
  private void initialize(int randsToSkip)
  {
    ...
    //测试句子数 = 总参考文件中的行数/测试文件中每个句子的对应的参考数
    ...
    numParams = countNonEmptyLines(paramsFileName) - 1;
      // the parameter file contains one line per parameter，子模型（参数）的数目
      // and one line for the normalization method


    paramNames = new String[1+numParams];
    lambda = new double[1+numParams]; // indexing starts at 1 in these arrays，参数
...
//    SentenceInfo.createV(); // uncomment ONLY IF using vocabulary implementation of SentenceInfo
    ...
    try {
      ...
      if(!metricName.equals("CHARACC") && !metricName.equals("MED") && !metricName.equals("TAGACC")) {
	      for (int i = 0; i < numSentences; ++i) {
	        for (int r = 0; r < refsPerSen; ++r) {
...
//    int[] lastUsedIndex = new int[numSentences];
    int[] maxIndex = new int[numSentences]; //每个句子选择的最优个数(20)
...
//    printMemoryUsage();
  ...
  /** 
   * 单个循环,包括特征的每次随机权重的最优值
   * <li> 
   * @since Jul 12, 2013
   * @param iteration 第几次循环
   * @param minIts 最少迭代次数
   * @param maxIts 最大迭代次数
   * @param prevIts 上一次迭代次数 
   * @param earlyStop 是否早停止
   * @param maxIndex 每个实例的最优候选值索引
   * @return
   */
  @SuppressWarnings("unchecked")
public double[] run_single_iteration(
    int iteration, int minIts, int maxIts, int prevIts, int earlyStop, int[]maxIndex)
  {
    ...
    double[] retA = new double[3];
      // retA[0]: FINAL_score，最终结果
      // retA[1]: earlyStop 是否早停
      // retA[2]: should this be the last iteration? 是不是最后一次迭代

    boolean done = false;
    retA[2] = 1; // will only be made 0 if we don't break from the following loop， 0表示不中断循环

    // featVal_array[1+numParams][numSentences][]，每个sentence的每个candidate的每个子模型的param的权重
    double[][][] featVal_array = new double[1+numParams][][];
      // indexed by [param][sentence][candidate]
    featVal_array[0] = null; // param indexing starts at 1
    for (int c = 1; c <= numParams; ++c) {
...
//      printMemoryUsage();
      ...
      //复制SDecoder_cfg.txt to SDecoder_cfg.txt.ZMERT.orig,得到子模型的权重lambda
      ...
      //分离nbest.out,得到临时文件，候选值和候选值的各项权重(ZMERT.temp.sents.itn(句子), 
      //ZMERT.temp.feats.itn（特征）) 
      produceTempFiles(decRunResult[0], iteration); //候选值和候选值的概率文件
     
      
      //复制SDecoder_cfg.txt 到 SDecoder_cfg.txt.ZMERT.itn
      ...
      //复制nbest.out 到 nbest.out.ZMERT.itn
      ...
      //每句候选的数目candCount，最后用的索引lastUsedIndex，和统计每个候选项的准确率suffStats_array
...
//        suffStats_array[i].clear();
      ...
      //初始权重initialLambda（每次初始点的每个权重，随机产生）, 和最终权重finalLambda
      ...
      try {
        ...
        /** 状态文件ZMERT.temp.stats.itn，如果存在就读取，如果不存在就创建 */
        //
        ...
        /** ZMERT.temp.stats.merged，存入 候选值的统计信息（准确率） */
        /** ZMERT.temp.stats.mergedKnown，一句话放一个||||||符号   */ 
        ...
        /** ZMERT.temp.currIt.unknownCands，存放新添加的Cand,每句话一行，每个句子之间用|||分割 */
        ...
        /** ZMERT.temp.currIt.unknownIndices, 存放每个候选对应的句子序列 */
        ...
        int[] sizeUnknown_currIt = new int[numSentences];//每个句子对应的未知候选的数目

        /** 这个for循环使用来读取sents和feats，来取得
         * ZMERT.temp.stats.mergedKnown,
         * ZMERT.temp.currIt.unknownCands, 
         * ZMERT.temp.currIt.unknownIndices。
         *  */
        //每个句子
...
//                println("",4);
          ...
          // now process the candidates of the current iteration 现在处理当前迭代的候选值
          // now determine the new candidates of the current iteration 现在确定当前迭代中新候选值
          ...
          */
          ...
          // 读取nbest.out（20<all), 将unknown的候选项存入unknown文件 
          for (int n = 0; n <= sizeOfNBest; ++n) {
          // Why up to and *including* sizeOfNBest?
          // So that it would read the "||||||" separator even if there is
          // a complete list of sizeOfNBest candidates.
            ...
            //System.out.println(sents_str);
            if (sents_str.equals("||||||")) {
              n = sizeOfNBest+1;
            } else if (!existingCandStats.containsKey(sents_str)) {
              unknownCands_V.add(sents_str);
              writeLine(sents_str,outFile_unknownCands);//写入unknownCands
              outFile_unknownIndices.println(i);        //写入unknownIndics
...
*/
        ...
        for (int it = firstIt; it < iteration; ++it) {
          inFile_sents[it].close();//关闭ZMERT.temp.sents.itn
          inFile_stats[it].close();//关闭ZMERT.temp.stats.itn
         ...
         * 对未见的候选值来计算统计值（准确率）,并存入ZMERT.temp.stats.unknown */
         ...
         *  打开ZMERT.temp.stats.unknown，
         *  打开 已合并的已知的候选值 的文件ZMERT.temp.stats.mergedKnown */
        ...
        /** 对于每个句子，每个候选值，统计其统计信息（准确率）,得到最优的
         *  decoder得到的最优score的初始值
         */
        for (int i = 0; i < numSentences; ++i) {
          ...
          // a complete list of sizeOfNBest candidates.
            ...
            sents_str = inFile_sentsCurrIt.readLine(); //读取sents
            feats_str = inFile_featsCurrIt.readLine(); //读取feats
...
*/
                ...
                outFile_statsCurrIt.println(stats_str);//输出到ZMERT.temp.stats.it
              } else {
              ...
              outFile_statsMerged.println(stats_str);//输出到ZMERT.temp.stats.merged
              //特征权重
              featVal_str = feats_str.split("\\s+");
...
//              println("",4);
              ...
              //根据随机初始值，计算decoder得到的最优score的初始值
              for (int j = 1; j <= initsPerIt; ++j) {
                double score = 0; // i.e. score assigned by decoder
                for (int c = 1; c <= numParams; ++c) {
                  score += initialLambda[j][c] * currFeatVal[c];
...
//        cleanupMemory();
      ...
      for (int j = 1; j <= initsPerIt; ++j) {
        ...
        //initialLambda: double[1+initsPerIt][1+numParams] 初始权重，随机产生
        //finalLambda: double[1+initsPerIt][1+numParams] 最终权重，进入optimizer前为0
        //best1Cand_suffStats: int[1+initsPerIt][numSentences][suffStatsCount]
        //  每个句子的最优候选（统计），这个是由decoder得到的（即特征权重和）
        //finalScore: double[1+initsPerIt]，为0
        //candCount: int[numSentences]，每句候选的数目candCount
        //featVal_array: featVal_array[1+numParams][numSentences][candCount]，
        //  每个sentence的每个candidate的每个子模型的param的权重
        //suffStats_array: new ConcurrentHashMap[numSentences];
        //  统计每个候选项的准确率suffStats_array
...
//      println("",2);
  ...
  /** 运行解码器./SDecoder_cmd (java SimpleDecoder SDecoder_cfg.txt src.txt nbest.out),
   * 得到nbest.out */
...
*/
  ...
  /** 分离nbest.out,得到候选值和候选值的各项权重(ZMERT.temp.sents.itn, ZMERT.temp.feats.itn)
   * <li> ZMERT.temp.sents.itn 包含候选句子
   * <li> ZMERT.temp.feats.itn 包含候选特征
   * @since Aug 12, 2013
   * @param nbestFileName
   * @param iteration
   */
...
*/
  ...
  /** 复制解码配置文件SDecoder_cfg.txt.ZMERT.orig 到 SDecoder_cfg.txt, 并存储现在特征（子模型）的权重lambda*/
  ...
  /**
   * 读取参数文件，进行特征（子模型）权重，设置lambda，optimization，maxThValue等值
   * @since Nov 6, 2012
   */
...
// normalization = LNorm 2 1
  ...
  /**
   * 设置句子对应的文档数，我们一般设为0
   * <p> sets numDocuments and docOfSentence[]
   * @since Nov 6, 2012
   */
...
*/
  ...
  /**
   * 根据ZMERT_cfg.txt配置文件 来 获得参数数组，以便用于处理
   * @since Oct 29, 2012
   * @param fileName
   * @return
   */
...
*/
    ...
    if (dirPrefix != null) { // append dirPrefix to file names，添加前缀目录
...
//    println("split_on: " + split_on);
  ...
  /**
   * 产生随机数，在minRandValue[c] maxRandValue[c]之间随机产生 
   * @since Oct 24, 2013
   * @return
   */
