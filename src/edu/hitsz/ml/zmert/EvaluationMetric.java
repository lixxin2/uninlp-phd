...
public abstract class EvaluationMetric
{
  ...
  protected int suffStatsCount; // number of sufficient statistics 有效统计的数目
  ...
  /** 
   * 设置评价标准，比如BLEU， CHARACC, MED, TAGACC
   * <p> CHARACC: 注音程序的评价标准，表示句子中字的准确率
   * <p> MED: 语音识别的评价标准，表示最小编辑距离
   * <p> TAGACC: tagging的评价标准，表示句子中tag的准确率
   * @since Nov 6, 2012
   */
...
//    metricOptionCount.put("PP_BLEU",4);
    metricOptionCount.put("CHARACC", 0);    
      //
    metricOptionCount.put("MED", 0);    
    //
    metricOptionCount.put("TAGACC", 0);    
  }
...
//      retMetric = new ParaphraseBLEU(metricOptions);   // the "PP_BLEU" metric corresponds to the ParaphraseBLEU class
    } else if (metricName.equals("CHARACC")) {
    	retMetric = new CHARACC(metricOptions);
    } else if (metricName.equals("MED")) {
    	retMetric = new MED(metricOptions);
    } else if (metricName.equals("TAGACC")) {
    	retMetric = new TAGACC(metricOptions);
    }
  ...
  /** 
   * 将候选句子的有效信息（准确率）存入文件outputFileName
   * @since Jul 17, 2013
   * @param cand_strings_fileName 候选句子
   * @param cand_indices_fileName 候选句子的索引
   * @param outputFileName 有效信息（准确率）文件
   * @param maxBatchSize 每个句子对应的候选句子的最大数
   */
    ...
    try {
      ...
      while (line_cand != null) {
        ...
        while (line_cand != null) {
          ...
          if(size > 0 && index != cand_indices[0]) break;
          cand_strings[size] = line_cand;
          //System.out.println(line_cand);
          cand_indices[size] = index;
        ...
        cand_strings = new String[maxBatchSize];
        cand_indices = new int[maxBatchSize];
      }
