...
public class IntermediateOptimizer implements Runnable
{
  ...
  private ConcurrentHashMap<Integer,int[]>[] suffStats_array;//每个句子的不重复的候选值（相交直线）的统计信息（错误率）
   ...
   * @param in_j 第j个初始点
   * @param in_blocker
   * @param in_threadOutput
   * @param in_initialLambda 初始权重
   * @param in_finalLambda 最终权重
   * @param in_best1Cand_suffStats int[1+initsPerIt][numSentences][suffStatsCount]
   * @param in_finalScore 各个初始点的最终值
   * @param in_candCount 每句的候选数目candCount
   * @param in_featVal_array 
   * @param in_suffStats_array 
   */
  ...
  /** 
   * 对特征c进行参数优化（ \alpha * c + d），得到所有句子的左右候选值的相交点
   * @since Oct 25, 2013
   * @param thresholdsAll key=交汇点的x，value=对应每个句子和两个候选值（就是交点相交的直线）
   * @param c 第c个特征
   * @param currLambda 权重
   * @param indicesOfInterest 每个句子的候选值的upper envolope对应的直线，比相交点的数目多一个
   */
...
//      println("Processing sentence #" + i,4);
      ...
      //保存每个句子的所有候选中的最小和最大slope（倾斜度），和对应的offset
      for (int k = 0; k < numCandidates; ++k) {
        slope[k] = featVal_array[c][i][k];         //要调整的参数（权重）对应的特征概率

        offset[k] = 0.0;                           //其他所有特征的概率和
...
//        print("cI=" + currIndex + " ",4);
        ...
        //记录在当前直线（与其他slope高的直线的交点）中最小的点，和相交的直线
        double nearestIntersectionPoint = PosInf;        
        int nearestIntersectingLineIndex = -1;
        //递归每个候选，如果slope大于当前currIndex的slope（即最小的slope）
        for (int k = 0; k < numCandidates; ++k) {
...
//    return thresholdsAll;
  ...
  /**
   * 通过每个交叉点的计算，得到特征c的最优权重（交叉点）和score
   * @since Oct 25, 2013
   * @param thresholdsAll key=交汇点的x，value=对应每个句子和两个候选值（就是交点相交的直线）
   * @param indexOfCurrBest 每个句子 权重*特征 和 最大的候选值序列
   * @param c
   * @param lambda
   * @return
   */
      ...
      int c, double[] lambda)
  {
    ...
    int[][] suffStats = new int[numSentences][suffStatsCount];
      ...
      // indicated by indexOfCurrBest[i]). 最优的
...
*/
  ...
  /** 
   * 统计每个句子的候选值（相交直线）的统计信息（错误率）
   * @param indicesOfInterest 每个句子的候选值（相交直线）
   */
...
//    println("(*_all: " + candsOfInterestCount_all + ")",2); *****/
    ...
    try {
      ...
      for (int i = 0; i < numSentences; ++i) {
        int numCandidates = candCount[i]; //每个句子不重复的候选值
  ...
  /** 
   * 根据当前权重temp_lambda，得到每个句子 权重*特征 和 最大的候选值序列
   * @since Oct 25, 2013
   * @param temp_lambda
...
//      }
  ...
  /**  
   * 进行一次调整，依次计算每个参数，返回结果最优的那个参数，以及其权重，对应的score
   * @since Oct 25, 2013
   * @param thresholdsAll
   * @param lastChanged_c
   * @param currLambda
...
//    cleanupMemory();
    ...
    //每个文档(我们设为1)的统计信息，计算当前每个文档的统计信息（将其中的句子的统计相加得到）
