package edu.hitsz.ml.zmert;

import java.math.*;
import java.util.*;
import java.io.*;
import java.text.DecimalFormat;

/** decode top_n from nbest list using the weights * features. 
 * <p> One for one sentence iteratively, to save memory.
 */
public class SimpleDecoderOne
{
  private static DecimalFormat f3 = new DecimalFormat("###0.000");
  public static void main(String[] args) throws Exception
  {
    String configFileName = args[0];
    String sourceFileName = args[1];
    String outputFileName = args[2];
    
    System.out.println("configFileName:" + configFileName);
    System.out.println("sourceFileName:" + sourceFileName);
    System.out.println("outputFileName:" + outputFileName);

    //模型的个数
    int numParams = 2;
    int numSentences = countLines(sourceFileName);

    double[] weights = new double[numParams];
    String candsFileName = ""; //候选集文件
    int cps = 0; //每个句子的候选集个数
    int N = 0; //最优个数

    InputStream inStream = new FileInputStream(new File(configFileName));
    BufferedReader inFile = new BufferedReader(new InputStreamReader(inStream, "utf8"));

    String line = inFile.readLine();
    while (line != null) {
      if (line.startsWith("cands_file")) {
        candsFileName = (line.substring(line.indexOf("=")+1)).trim();
      } else if (line.startsWith("cands_per_sen")) {
        cps = Integer.parseInt((line.substring(line.indexOf("=")+1)).trim());
      } else if (line.startsWith("top_n")) {
        N = Integer.parseInt((line.substring(line.indexOf("=")+1)).trim());
      } else if (line.startsWith("submodel00")) {
        weights[0] = Double.parseDouble((line.substring(10+1)).trim());
      } else if (line.startsWith("submodel01")) {
        weights[1] = Double.parseDouble((line.substring(10+1)).trim());
      } else if (line.startsWith("submodel02")) {
        weights[2] = Double.parseDouble((line.substring(10+1)).trim());
      } else if (line.startsWith("submodel03")) {
        weights[3] = Double.parseDouble((line.substring(10+1)).trim());
      } else if (line.startsWith("submodel04")) {
        weights[4] = Double.parseDouble((line.substring(10+1)).trim());
      } else if (line.startsWith("submodel05")) {
        weights[5] = Double.parseDouble((line.substring(10+1)).trim());
      } else if (line.startsWith("submodel06")) {
        weights[6] = Double.parseDouble((line.substring(10+1)).trim());
      } else if (line.startsWith("submodel07")) {
        weights[7] = Double.parseDouble((line.substring(10+1)).trim());
      } else if (line.startsWith("submodel08")) {
        weights[8] = Double.parseDouble((line.substring(10+1)).trim());
      } else if (line.startsWith("submodel09")) {
        weights[9] = Double.parseDouble((line.substring(10+1)).trim());
      } else if (line.startsWith("submodel10")) {
        weights[10] = Double.parseDouble((line.substring(10+1)).trim());
      } else if (line.startsWith("#")) {
      } else if (line.length() > 0) {
        println("Wrong format in config file.");
        System.exit(1);
      }
      line = inFile.readLine();
    }

    inFile.close();
    
    for(int i=0; i<numParams; i++)
      System.out.print(weights[i] + ",");
    System.out.println();

    //String[][] candidates = new String[numSentences][cps];
    //double[][][] features = new double[numSentences][cps][numParams];
    String[] candidatesOne = new String[cps];
    double[][] featuresOne = new double[cps][numParams];
    
    //double[][] scores = new double[numSentences][cps];
    double[] scoresOne = new double[cps];
    
    inStream = new FileInputStream(new File(candsFileName));
    inFile = new BufferedReader(new InputStreamReader(inStream, "utf8"));
    
    FileOutputStream outStream = new FileOutputStream(outputFileName, false); // false: don't append
    OutputStreamWriter outStreamWriter = new OutputStreamWriter(outStream, "utf8");
    BufferedWriter outFile = new BufferedWriter(outStreamWriter);

    System.out.print("sentence: ");
    for (int i = 0; i < numSentences; ++i) {
      System.out.print(i + ",");
      for (int n = 0; n < cps; ++n) {
        // read the nth candidate for the ith sentence
        line = inFile.readLine();

/*
line format:

i ||| words of candidate translation . ||| feat-1_val feat-2_val ... feat-numParams_val .*

*/

        line = (line.substring(line.indexOf("|||")+3)).trim(); // get rid of initial text

        String candidate_str = (line.substring(0,line.indexOf("|||"))).trim();
        String feats_str = (line.substring(line.indexOf("|||")+3)).trim(); // get rid of candidate string

        int junk_i = feats_str.indexOf("|||");
        if (junk_i >= 0) {
          feats_str = (feats_str.substring(0,junk_i)).trim();
        }

        String[] featVal_str = feats_str.split("\\s+");

        candidatesOne[n] = candidate_str;
        for (int c = 0; c < numParams; ++c) {
          featuresOne[n][c] = Double.parseDouble(featVal_str[c]);
        }
      }


      for (int n = 0; n < cps; ++n) {
        scoresOne[n] = 0;
        for (int c = 0; c < numParams; ++c) {
          scoresOne[n] += weights[c]*featuresOne[n][c];
        }
      }
   

      int[] indices = sortOne(scoresOne);
      for (int n = 0; n < N; ++n) {
        String str = "" + i + " ||| " + candidatesOne[indices[n]] + " |||";
        for (int c = 0; c < numParams; ++c) {
          str += " " + f3.format(featuresOne[indices[n]][c]);
        }
        str += " ||| " + f3.format(scoresOne[indices[n]]);
        writeLine(str, outFile);
      }

    }

    outFile.close();

    System.exit(0);

  }

  private static int[] sort(double[][] scores, int i)
  {
    int numCands = scores[i].length;
    int[] retA = new int[numCands];
    double[] sc = new double[numCands];

    for (int n = 0; n < numCands; ++n) {
      retA[n] = n;
      sc[n] = scores[i][n];
    }

    for (int j = 0; j < numCands; ++j) {
      int best_k = j;
      double best_sc = sc[j];
      for (int k = j+1; k < numCands; ++k) {
        if (sc[k] > best_sc) {
          best_k = k;
          best_sc = sc[k];
        }
      }

      // switch j and best_k
      int temp_n = retA[best_k];
      retA[best_k] = retA[j];
      retA[j] = temp_n;

      double temp_sc = sc[best_k];
      sc[best_k] = sc[j];
      sc[j] = temp_sc;
    }

    return retA;
  }
  
  private static int[] sortOne(double[] scores)
  {
    int numCands = scores.length;
    int[] retA = new int[numCands];
    double[] sc = new double[numCands];

    for (int n = 0; n < numCands; ++n) {
      retA[n] = n;
      sc[n] = scores[n];
    }

    for (int j = 0; j < numCands; ++j) {
      int best_k = j;
      double best_sc = sc[j];
      for (int k = j+1; k < numCands; ++k) {
        if (sc[k] > best_sc) {
          best_k = k;
          best_sc = sc[k];
        }
      }

      // switch j and best_k
      int temp_n = retA[best_k];
      retA[best_k] = retA[j];
      retA[j] = temp_n;

      double temp_sc = sc[best_k];
      sc[best_k] = sc[j];
      sc[j] = temp_sc;
    }

    return retA;
  }

  private static void sort(int[] keys, double[] vals, int start, int end)
  {
    if (end-start > 1) {
      int mid = (start+end)/2;
      sort(keys,vals,start,mid);
      sort(keys,vals,mid+1,end);

    }
  }

  private static int countLines(String fileName) throws Exception
  {
    BufferedReader inFile = new BufferedReader(new FileReader(fileName));

    String line;
    int count = 0;
    do {
      line = inFile.readLine();
      if (line != null) ++count;
    }  while (line != null);

    inFile.close();

    return count;
  }

  private static void writeLine(String line, BufferedWriter writer) throws IOException
  {
    writer.write(line, 0, line.length());
    writer.newLine();
    writer.flush();
  }

  private static void println(Object obj) { System.out.println(obj); }
  private static void print(Object obj) { System.out.print(obj); }

}


